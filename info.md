...
- **Hybrid режимом**: cloud-first → автоматический fallback на local при потере связи или когда облако давно не получало данных от контроллера
- **Local-only режимом** — для тех, у кого вообще нет интернета

Локальное подключение работает через внутренние запросы веб-интерфейса контроллера — это не официальный API MyHeat, после обновления прошивки он может измениться.
