...
При включённом локальном подключении (гибридный режим или «Только локально») интеграция отдельно опрашивает контроллер **каждые 15 секунд** (настраивается, 5–300 с). Ошибка на каждое включение и выключение — не больше половины интервала, в 4 раза точнее облака.
