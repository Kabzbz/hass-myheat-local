...
При включённом локальном подключении интеграция отдельно опрашивает контроллер **каждые 15 секунд** (настраивается, 5–300 с) и ведёт свой учёт. Ошибка на каждое включение и выключение — не больше половины интервала, в 4 раза точнее облака:

- `binary_sensor.<имя>_котел_пламя` — горит ли горелка прямо сейчас; в атрибутах: на отопление или на ГВС, насос, связь с котлом, ошибка котла
- `sensor.<имя>_котел_время_работы_горелки` — часы, накопительный счётчик для статистики HA, переживает перезапуск
- `sensor.<имя>_котел_розжиги_горелки` — число розжигов; частые короткие циклы (много розжигов при малом времени работы) — признак, что котёл «тактует»

Выбег насоса после погасания пламени как работа горелки не считается.
