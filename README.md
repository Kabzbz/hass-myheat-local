...
- поменять IP, логин, пароль, протокол контроллера
...
- `sensor.<имя>_котел_розжиги_горелки` — число розжигов; много розжигов при малом времени работы — признак, что котёл «тактует»
- `sensor.<имя>_котел_включения_гвс` — сколько раз котёл начинал греть воду (если кран открыли во время отопления, котёл переключается без погасания пламени — это включение ГВС, но не новый розжиг)
...
Чего контроллер локально **не отдаёт**: модуляцию горелки, целевую температуру котла и уличную температуру — в режиме «Только локально» эти датчики показывают «Неизвестно». Подача, обратка и давление котла приходят и локально.
...
Учёт времени горелки точный, поэтому газ считается как **часы работы горелки × расход котла**. Включается в **«Настроить» → «Расход газа при работе горелки, м³/ч»** (0 — не считать). Появятся:

- `sensor.<имя>_котел_газ_оценка` — всего, м³ — его и добавлять в **Настройки → Панели → Энергия → Потребление газа**
- `sensor.<имя>_котел_газ_на_отопление_оценка` и `…_газ_на_гвс_оценка`

**Как узнать свой расход** (лучше, чем паспортный максимум, потому что котёл обычно работает не на полной мощности): запишите показания газового счётчика и значение «Время работы горелки», через неделю — ещё раз. Расход = (м³ по счётчику) / (часы горелки). Если позже поменяете число, новое действует с этого момента, накопленное не пересчитывается.
